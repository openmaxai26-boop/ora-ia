...
const agents = [
  {
    ...
    features: ["Publication automatique", "Visuels générés par IA", "Contenu en français & tahitien", "Statistiques en temps réel"],
  },
  {
    name: "Hina",
    emoji: "💬",
    ...
    features: ["Disponible 24h/24 7j/7", "WhatsApp & Messenger", "Prise de rendez-vous", "Escalade vers humain si besoin"],
  },
  {
    name: "Reva",
    emoji: "✍️",
    ...
    features: ["30 articles/mois", "Optimisation SEO Google", "Adapté au marché local", "Pub sur votre site automatiquement"],
  },
  {
    name: "Manu",
    emoji: "👔",
    ...
    features: ["Publication des offres", "Tri automatique des CV", "Score des candidats", "Rapport de présélection"],
  },
  {
    name: "Ari",
    emoji: "🎯",
    ...
    features: ["Ciblage intelligent", "Messages personnalisés", "Suivi des relances", "Rapport hebdomadaire"],
  },
];
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {agents.map((agent) => (
            ...
            >
              {/* Agent avatar */}
              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${agent.color} flex items-center justify-center text-3xl mb-6`}>
                {agent.emoji}
              </div>
              ...
              <ul className="space-y-2">
                {agent.features.map((f) => (
                  <li key={f} className="flex items-center gap-2 text-sm text-ocean/70">
                    <span className="w-4 h-4 bg-lagoon/20 text-lagoon rounded-full flex items-center justify-center text-xs font-bold">✓</span>
                    {f}
                  </li>
                ))}
              </ul>
