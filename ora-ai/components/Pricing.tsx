      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-ocean/60 text-xl max-w-2xl mx-auto">
            Tarifs en Francs CFP. Sans engagement. Éligible à l&apos;Aide à la Création Numérique (ACN) de la DGEN.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 items-start">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative border-2 ${plan.color} rounded-2xl p-8 card-hover ${plan.badge ? "shadow-xl" : "shadow-sm"}`}
            >
              {/* Badge */}
              {plan.badge && (
              ...
              <div className="mb-6">
                <h3 className="text-xl font-bold text-ocean mb-1">{plan.name}</h3>
                <p className="text-ocean/50 text-sm mb-4">{plan.description}</p>
                <div className="flex items-baseline gap-1">
                  <span className="text-4xl font-black text-ocean">{plan.price}</span>
                  <span className="text-ocean/50 font-medium">{plan.currency}{plan.period}</span>
                </div>
              </div>
        ...
        <div className="mt-12 bg-ocean/5 border border-ocean/10 rounded-2xl p-6 text-center">
          <p className="text-ocean/70">
            💡 <strong>Bon à savoir :</strong> L&apos;Aide à la Création Numérique (ACN) de la DGEN peut financer
            une partie de votre abonnement Ora AI. Renseignez-vous auprès de{" "}
            <span className="text-lagoon font-semibold">la Direction Générale de l&apos;Économie Numérique</span>.
          </p>
        </div>
      </div>
    </section>
