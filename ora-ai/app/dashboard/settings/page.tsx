...
export default function SettingsPage() {
  ...
  // ——— Champs notifications (formulaire local) ———
  ...
  const handleSave = async () => {
    ...
    try {
      // Sauvegarder profil + notifications en parallèle
      ...
      if (!profileResult.success) {
        throw new Error(profileResult.error || "Erreur sauvegarde profil");
      }
      if (!notifResult.success) {
        throw new Error(notifResult.error || "Erreur sauvegarde notifications");
      }
  ...
  const toggleNotif = async (key: keyof typeof notifForm) => {
    const newValue = !notifForm[key];
    const newForm = { ...notifForm, [key]: newValue };
    setNotifForm(newForm);

    // Sauvegarde immédiate pour les toggles
        ...
        <div className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setProfileForm((p) => ({ ...p, langue: e.target.value }))
                  }
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-ocean focus:outline-none focus:ring-2 focus:ring-lagoon/40 focus:border-lagoon transition-all text-sm"
                >
            ...
            <div className="space-y-4">
              {[
                ...
                >
                  ...
                  >
                    <span
                      className={"absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all " + (notifForm[notif.key] ? "left-7" : "left-1")}
                    />
                  </button>
                </div>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="flex items-center justify-between p-5 bg-lagoon/5 border border-lagoon/20 rounded-xl mb-4">
              <div>
                <div className={"font-black text-xl " + planInfo.color}>
                  Plan {planInfo.label}
                </div>
              ...
              <div className="text-right">
                {subscription?.plan !== "business" && (
                  ...
                  >
                    {subscription?.plan === "starter" ? "Passer au Pro" : "Passer au Business"}
                  </a>
                )}
                <a
                  href="#"
                  className="text-ocean/40 text-xs hover:text-red-400 transition-colors"
                >
